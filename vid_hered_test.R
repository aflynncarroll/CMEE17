...
# # converts into date
# 
# soc$year <- data$date[match(soc$individual, data$id1)]
# soc$year <- data$date[match(soc$individual, data$id2)]
# # add sample year
# # change depending on when data collected
# 
# soc$age <- as.numeric(soc$year)-soc$cohort
...
# 
# 
# days <- data.matrix(unique(data$date, incomparables = FALSE, MARGIN = 1, fromLast = FALSE,))
