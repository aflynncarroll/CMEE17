...
# format and save data from iqbal 
...
tag_codes <- read.csv('../Data/tblAllCodes.csv', header=T)

tran_code <- subset(tag_codes, CodeType == 'T')
...
data <- read.csv(paste('../Results/',f_name,'.csv', sep=''), header=T)
# video data file 
...
data$individual1 <- tran_code$BirdID[match(data$id1, tran_code$Code)]
# change bird id1 to ID code from ring data

data$individual2 <- tran_code$BirdID[match(data$id2, tran_code$Code)]
...
soc_trip <- function(ev){ 
  ...
  #soc$cohort <- sparrows$Cohort[match(soc$individual, sparrows$BirdID)]
  # adds cohort to the measured individuals
  
  #soc$year <- df$date[1]
  #soc$year<-format(as.Date(df$Date[1], format="%Y-%m-%d"),"%Y")
  soc$year<- 2017
...
#soc_total <- rbind(soc_t1,soc_t2,soc_t3,soc_t4,soc_t5,soc_t6,soc_t7,soc_t8)

write.csv(soc_t1, '../Results/iqbal_soc.csv')
#write.csv(soc_total, '../Results/iqbal_soc.csv')
