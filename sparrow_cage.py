...
# run inside_cage2.py 'Data/20171230.CSV' 'sorted_test.csv'
# run sparrow_cage.py '../Data/20171130.CSV' '20171130'
...
#cage_data.sort_values(by=cage_data['date_time2'])
...
############################################################################################
# Add anti-social birds 
############################################################################################
####experiments
# hung
# hungry = pd.DataFrame(pd.unique(cage_data2.id), columns=['id'])

# new= pd.DataFrame(['007fake'], columns=['id'])

# hungry = hungry['id'].append(new.id)
# hungry.columns = ['id']

# social = pd.DataFrame(pd.unique(pd.concat([overlap["id1"],overlap["id2"]])), columns=['id'])


# hungry.isin(social)
# common['match'] = hungry.loc[hungry[0] != social[0]] 

# common = hungry[~hungry.index.isin(social.index)]

# dom_data = dom_data[~dom_data.individual1.isin(bird_id.Code) == False]

# hungry = hungry[~hungry.isin(social.id)==True]

# common = pd.DataFrame(social[~social.isin(hungry.id) == True], columns=['id'])
...
# right now all interactions (even 0s) are saved to a file
