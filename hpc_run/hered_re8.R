...
dat_t<- plyr::rename(dat_t, c("individual" = "animal"))
...
if(iter==1){
  mt <- MCMCglmm(cbind(closeness_norm, density)~, random=~animal+BirdID+trip+mat_id+soc_brood, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # account individual variation :closeness
  save(mt, file= 'mt_c8.rda')
}

if(iter==2){
  mt <- MCMCglmm(cbind(betweenness_norm, density)~, random=~animal+BirdID+trip+mat_id+soc_brood, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # betweenness
  save(mt, file= 'mt_b8.rda')
}

if(iter==3){
  mt <- MCMCglmm(cbind(degree_norm, density)~, random=~animal+BirdID+trip+mat_id+soc_brood, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # degree
  save(mt, file= 'mt_d8.rda')
}
