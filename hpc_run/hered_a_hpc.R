...
if(iter==1){
  mt <- MCMCglmm(closeness_norm~1, random=~animal+BirdID+age, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # account individual variation :closeness
  save(mt, file= 'mt_c_a.rda')
}

if(iter==2){
  mt <- MCMCglmm(betweenness_norm~1, random=~animal+BirdID+age, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # betweenness
  save(mt, file= 'mt_b_a.rda')
}

if(iter==3){
  mt <- MCMCglmm(degree_norm~1, random=~animal+BirdID+age, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # degree
  save(mt, file= 'mt_d_a.rda')
}


if(iter==4){
  mt <- MCMCglmm(closeness_norm~1, random=~animal+BirdID+trip+mat_id+soc_brood+age, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # account individual variation :closeness
  save(mt, file= 'mt_c5a.rda')
}

if(iter==5){
  mt <- MCMCglmm(betweenness_norm~1, random=~animal+BirdID+trip+mat_id+soc_brood+age, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # betweenness
  save(mt, file= 'mt_b5a.rda')
}

if(iter==6){
  mt <- MCMCglmm(degree_norm~1, random=~animal+BirdID+trip+mat_id+soc_brood+age, data=dat_f, ped=fp, nitt=1000000, burnin=10000, verbose=FALSE)
  # degree
  save(mt, file= 'mt_d5a.rda')
}
